  ...
  async profiles(@Res() response: any): Promise<any> {
    ...
    for (let i = 0; i < data.items.length; i++) {
      ...
      const row = {
        ...
        role: item.role,
        native_language: item.languages.nativeLanguage,
        learning_language: item.languages.learningLanguage,
        level: item.languages.learningLanguageLevel,
        goal: item.preferences.goals,
        interests: item.personalInformation.interests,
