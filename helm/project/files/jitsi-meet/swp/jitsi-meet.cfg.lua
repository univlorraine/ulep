...
{{ $TURN_USERNAME := .Env.TURN_USERNAME | default "" -}}
{{ $TURN_PASSWORD := .Env.TURN_PASSWORD | default "" -}}
...
external_services = {
  ...
  {{- if $TURN_HOST -}}
    {{- range $idx1, $host := $TURN_HOSTS -}}
      {{- range $idx2, $transport := $TURN_TRANSPORTS -}}
        ...
        { type = "turn", host = "{{ $host }}", port = {{ $TURN_PORT }}, transport = "{{ $transport }}", username = "{{ $TURN_USERNAME }}", password = "{{ $TURN_PASSWORD }}", secret = true, ttl = {{ $TURN_TTL }}, algorithm = "turn" }
      {{- end -}}
    {{- end -}}
  {{- end -}}
  ...
  {{- if $TURNS_HOST -}}
    {{- range $idx, $host := $TURNS_HOSTS -}}
        ...
        { type = "turns", host = "{{ $host }}", port = {{ $TURNS_PORT }}, transport = "tcp", username = "{{ $TURN_USERNAME }}", password = "{{ $TURN_PASSWORD }}", secret = true, ttl = {{ $TURN_TTL }}, algorithm = "turn" }
    {{- end }}
  {{- end }}
};
{{- end }}
