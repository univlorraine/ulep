...
class CreateReportUsecase implements CreateReportUsecaseInterface {
    ...
    async execute(reportCategoryId: string, content: string): Promise<undefined | Error> {
        try {
            const httpResponse: HttpResponse<undefined> = await this.domainHttpAdapter.post(`/reports/`, {
